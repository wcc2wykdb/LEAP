...
class UrlHash
{
    friend struct UrlHashHasher;

public:
    UrlHash() {}
    ~UrlHash() {}
    UrlHash &
    operator=(const UrlHash &h)
    {
...
struct UrlHashHasher {
    bool
    operator()(const UrlHash *s1, const UrlHash *s2) const
    {
        return 0 == memcmp(s1->_hash, s2->_hash, sizeof(s2->_hash));
    }

    size_t
    operator()(const UrlHash *s) const
    {
...
//typedef std::pair<UrlHash, double> UrlEntry;
using UrlList = std::list<UrlEntry>;
typedef std::unordered_map<const UrlHash *, UrlList::iterator, UrlHashHasher, UrlHashHasher> UrlMap;
static UrlEntry NULL_URL_ENTRY;
...
public:
    ...
    void reset();
    void updateMaxUtility(double new_value);
    void incrementScreenCounter(int &count);
    void incrementPrefetchCounter(bool actual);
    bool updateQoeg(const String &url, double qoeg);
    void ResetQoeGain();
    void incrementBandwidth(int value);
    void decrementBandwidth(int value);
private:
    ...
    /*prefetch condition*/
public:
    int _occupiedBandwidth;
    int _residualBandwidth;
    TSMutex _bandwidthLock;

    int _screenPrefetchCounter;
    int _totalPrefetchCounter;
    int _screenedPrefetchCounter;
    TSMutex _prefetchCounterLock;

    int _r;
    TSMutex _rLock;
    double _maxUtility;
    TSMutex _maxUtilityLock;
    int _n;
    int _k;
    double _b;

public:
    TSMutex _listAndMapLock;
    UrlMap _map;
    UrlList _list;
    UrlMap _reservemap;
    UrlList _reservelist;
};
