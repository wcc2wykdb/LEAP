...
public:
    PrefetchTxnData(PrefetchInstance *inst)
            : _inst(inst), _front(true), _firstPass(true),_miss(false), _fetchable(false), _status(TS_HTTP_STATUS_OK)
    {
    }
...
foundFresh(TSHttpTxn txnp)
{
    bool fresh = false;
    int lookupStatus;
    if (TS_SUCCESS == TSHttpTxnCacheLookupStatusGet(txnp, &lookupStatus)) { // if cached
//        PrefetchDebug("lookup status: %s", getCacheLookupResultName((TSCacheLookupResult)lookupStatus));
        if (TS_CACHE_LOOKUP_HIT_FRESH == lookupStatus) { // if fresh
            fresh = true;
        }
    } else {
...
isFetchable(TSHttpTxn txnp, PrefetchTxnData *data)
{
    ...
    if (!foundFresh(txnp)) { //if not cached or not fresh
    ...
    switch (event) {
        case TS_EVENT_HTTP_POST_REMAP: {
            ...
            if (!appendCacheKey(txnp, reqBuffer, data->_cachekey)) {
                PrefetchError("to get the cache key");
                TSHttpTxnReenable(txnp, TS_EVENT_HTTP_ERROR);
                return 0;
            }
        ...
        case TS_EVENT_HTTP_CACHE_LOOKUP_COMPLETE: {
            if (data->frontend()) { //frontend
                /* front-end instance */
                if (data->secondPass()) {//prefetch request
                    /* second-pass */
                    data->_fetchable = state->acquire(data->_cachekey);
                    ...
                    if (!foundFresh(txnp)) { //cache miss
                        PrefetchDebug("request cache miss");

                        size_t start = data->_cachekey.find("avc1/") + 5;
                        String bitrate_str =  data->_cachekey.substr(start,1);
                        int bitrate_index= atoi(bitrate_str.c_str());
                        int bitrate_set[]= {350,600,1000,2000,3000}; //unit kbps
                        data->_bitrate = bitrate_set[bitrate_index-1];
                        state->incrementBandwidth(data->_bitrate);
                        data->_miss = true;

                        if(state->_reservelist.size()>=int(5*1024/0.6787)){
                            const char* url;
                            int url_len = 0;
                            UrlHash hash;
                            UrlMap::iterator it;

                            url = data->_cachekey.c_str();
                            hash.init(url, url_len);
                            TSMutexLock(state->_listAndMapLock);
                            it = state->_reservemap.find(&hash);
                            if(it == state->_reservemap.end()){
                                TSMutexUnlock(state->_listAndMapLock);
                ...
                if (data->secondPass()) {
                    if(state->_reservelist.size()>=int(5*1024/0.6787)){
                        ...
                        if(it == state->_reservemap.end()){
                            ...
                            TSMBuffer bufp;
                            TSMLoc hdrLoc;
                            const char *value = "max-age=5"; //5 second later, cache expires.
                            int valueLen = 9;

                            if (TS_SUCCESS == TSHttpTxnServerRespGet(txnp, &bufp, &hdrLoc)) {
                                if (removeHeader(bufp, hdrLoc, TS_MIME_FIELD_CACHE_CONTROL, TS_MIME_LEN_CACHE_CONTROL)) {
                                    PrefetchDebug("remove header '%.*s'", (int)TS_MIME_LEN_CACHE_CONTROL, TS_MIME_FIELD_CACHE_CONTROL);
                                }
                                if (setHeader(bufp, hdrLoc, TS_MIME_FIELD_CACHE_CONTROL, TS_MIME_LEN_CACHE_CONTROL, value, valueLen)) {
                                    PrefetchDebug("set header '%.*s: %.*s'", TS_MIME_LEN_CACHE_CONTROL, TS_MIME_FIELD_CACHE_CONTROL, valueLen, value);
                                }
        ...
        case TS_EVENT_HTTP_SEND_RESPONSE_HDR: {
            if (data->frontend()) {
                PrefetchDebug("data->firstPass()=%s",data->firstPass()?"true":"false");
                PrefetchDebug("data->_fetchable=%s",data->_fetchable?"true":"false");
                PrefetchDebug("config.getNextPath().empty()=%s",config.getNextPath().empty()?"true":"false");
                PrefetchDebug("respToTriggerPrefetch(txnp)=%s",respToTriggerPrefetch(txnp)?"true":"false");
                        ...
                        for (unsigned i = 0; i < total; ++i) {
                            ...
                            if (config.getNextPath().replace(currentPath, expandedPath)) {
                                ...
                                BgFetch::schedule(state, config, /* askPermission */ false, reqBuffer, reqHdrLoc, txnp, expandedPath.c_str(),
                                                  expandedPath.length(), data->_cachekey, data->_prefetchHeader);
                            } else {
        ...
        case TS_EVENT_HTTP_TXN_CLOSE: {
            if (data->_fetchable) {
                if (data->frontend()) {
                    /* front-end */
                    if (data->firstPass()) {
                        PrefetchDebug("http txn close：frontend + firstPass");
                        /* first-pass */
                        if (!config.isExactMatch()) {
                        ...
                        /* second-pass */
                        PrefetchDebug("http txn close：frontend + secondPass");
            ...
            if (data->frontend()) {
                if (data->firstPass()) {
                    if (data->_miss){
                        state->decrementBandwidth(data->_bitrate);
                    }
                }
            }
...
{
    PrefetchInstance *inst = (PrefetchInstance *)instance;
    char clientInfoHeader[2000];
    memset(clientInfoHeader, 0, sizeof(clientInfoHeader));
    int clientInfoHeaderLen = 0;
    ...
    if (nullptr != inst) {
        ...
        const String prefetchHeader = "Prefetch";
        if (nullptr != method && methodLen == TS_HTTP_LEN_GET && 0 == memcmp(TS_HTTP_METHOD_GET, method, TS_HTTP_LEN_GET)) {
            if (headerExist(rri->requestBufp, rri->requestHdrp, prefetchHeader.c_str(), prefetchHeader.length())) {
                getHeader(rri->requestBufp, rri->requestHdrp, prefetchHeader.c_str(), prefetchHeader.length(), clientInfoHeader, &clientInfoHeaderLen);
            }
            else{
                PrefetchDebug("Prefetch Header not found");
            }
            ...
            if (handleFetch) {
                ...
                if (nullptr != data) {
                    ...
                    data->_prefetchHeader = clientInfoHeader;
